...
export function registerResumenCommand(bot: Telegraf<BotContext>): void {
  bot.hears(commandTrigger('resumen'), async (ctx) => {
    const target = resolveTarget(ctx);

    if (!target) {
      await ctx.reply(
        '⚠️ Debes *responder* a un mensaje con texto, audio, imagen, video o documento con `/resumen`\\.',
    ...
    try {
      ...
      console.error(`[ERROR] /resumen: ${msg}`);
