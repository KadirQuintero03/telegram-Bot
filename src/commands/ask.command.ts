...
export function registerAskCommand(bot: Telegraf<BotContext>): void {
  bot.hears(commandTrigger('ask'), async (ctx) => {
    const prompt = getCommandArgs(ctx.message.text).trim();

    if (!prompt) {
      await ctx.reply('⚠️ Debes escribir una pregunta. Ejemplo: `/ask Cómo hacer un asado?`', {
        parse_mode: 'Markdown',
      });
