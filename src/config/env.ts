...
export const config = {
  ...
  // Ruta donde se guarda registry.json (datos de usuarios). Se mantiene FUERA
  // de filesStoragePath a propósito: filesStoragePath se sirve públicamente a
  // través del explorer server (usado por GlowPic), así que guardar ahí el
  // registro de usuarios (con sus teléfonos, IDs, etc.) lo dejaría accesible
  // desde la web con solo adivinar la ruta. Por defecto se guarda en
  // "./data" (raíz del proyecto, junto a src/, no dentro de filesBotTelegram).
  ...
  // Usada por !tran, !resume y !ask. No se usa requireEnv() para que el
